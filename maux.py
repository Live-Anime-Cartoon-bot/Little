...
async def _verification_required(message: Message) -> bool:
    ...
    Rules:
      ...
      * Normal users in the allowed group require an active verification token/access.
    ...
    # Normal users in the explicitly allowed group require an active token.
    # /Token remains a separate feature used to create that access link.
    ...
    await message.reply_text(
        "🔒 **Token Required**\n\n"
        "This command requires an active verification token.\n\n"
        "Use `/Token` to create your access link.",
        reply_markup=InlineKeyboardMarkup([
...
async def start(client, message):
    ...
    await message.reply_text("🎬 **Welcome to Video Recorder Bot!**\n\n🔐 Verification is required for recording commands.\n🔑 Use /token to generate a verification token.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔑 Token Generator", callback_data="token_generate")],[InlineKeyboardButton("📖 Help", callback_data="help")],[InlineKeyboardButton("💠 Plans", callback_data="plan")]]))
...
async def rec_command(client, message: Message):
    ...
    force_16_9 = False
    if args and str(args[0]).strip().casefold() == "-16:9":
        force_16_9 = True
        args = args[1:]
...
async def _download_tool_media(client, info, path, status):
    ...
    def progress_callback(current, total):
        try:
            asyncio.create_task(_tool_download_progress(status, current, total))
        except Exception:
            pass
    ...
    try:
        ...
        if mode == "169":
            command = [
                ...
                "-pix_fmt", "yuv420p", "-aspect", "16:9", "-threads", "2",
            ...
            is_image_url = watermark_value.lower().startswith((
                "http://", "https://"
            )) and any(ext in watermark_value.lower().split("?", 1)[0] for ext in (
                ".png", ".jpg", ".jpeg", ".webp"
            ))
            ...
            if is_image_url:
                ...
                with urllib_request.urlopen(watermark_value, timeout=30) as resp:
                    image_path.write_bytes(resp.read())

                if mode == "watermark2":
                    overlay = "[base][watermark]overlay=20:main_h-overlay_h-60[outv]"
                else:
                    overlay = "[base][watermark]overlay=(main_w-overlay_w)/2:(main_h-overlay_h)/2[outv]"
                ...
                if mode == "watermark2":
                    position = "x=20:y=h-th-60"
                elif mode == "watermark3":
                    position = "x=(w-tw)/2:y=(h-th)/2"
                else:
                    position = "x=(w-tw)/2:y=h-th-140"
                vf = (
                    "scale=1920:1080,setsar=1,drawtext="
...
@app.on_callback_query(filters.regex(r"^vtool:(audio|trim|watermark|screenshot|169|169bars)$"))
async def video_tools_callback(client, query):
    ...
    if not _is_owner(uid) and not (bot_settings.get("premium_access", True) and _is_premium(uid)):
        ...
        if int(chat_id_for_access or 0) not in ALLOWED_REC_GROUP_IDS or (bot_settings.get("token", True) and not _has_valid_access(uid)):
            return await query.answer("🔒 Token required. Use /Token first.", show_alert=True)
    menu_state = None
    menu_key = None
...
async def video_tools_text_input(client, message: Message):
    ...
    if mode in ("trim", "watermark1", "watermark2", "watermark3") and "range" not in state:
        ...
        if rng and len(lines) >= 2:
            ...
            if mode.startswith("watermark") and not watermark_value:
            ...
            if mode.startswith("watermark"):
                state["watermark_text"] = watermark_value
        ...
        # Watermark uses ONE reply only. Do not ask separate Rename File or
        # Watermark Text/Link questions. The user must send all three lines.
        if mode.startswith("watermark"):
            return await message.reply_text(
                ...
                "-RenameFile Little Singham\n"
                "-WatermarkText/link LittleSinghamChannel`\n\n"
                "For an image watermark, put the image URL after `-WatermarkText/link`."
            )
