...
# /rec process UI
PROCESS_PREVIEW_INTERVAL = 20
PROCESS_LOADING_GIF_URL = "https://media.tenor.com/XasjKGMk_wAAAAAM/load-loading.gif"
PROCESS_LOADING_PHOTO_PATH = "/tmp/rec_loading_preview.jpg"
PROCESS_PREVIEW_DIR = "/tmp/rec_live_previews"
...
async def _auto_updater(task_id: str, msg, progress_file: str | None,
    ...
    try:
        while True:
            ...
            try:
                if info.get("process_photo", False):
                    await msg.edit_caption(
                        caption=_build_process_photo_caption(info),
                        parse_mode=ParseMode.MARKDOWN,
                        reply_markup=keyboard,
                    )
                else:
                    await msg.edit_text(
                        text,
                        parse_mode=None
                        if info.get("kind") in {"media", "stream_extractor", "default_audio"}
...
async def rec_cancel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if info.get("kind") == "default_audio":
        ...
        return
    if info.get("kind") not in {"merge", "stream_extractor", "default_audio"}:
        # Recording cancellation: stop FFmpeg/updaters now and delete the partial output.
        ...
        return
    message = (
        "⏳ Cancelling merge..." if info.get("kind") == "merge"
...
async def start_recording(stream_url: str, duration_seconds: int, output_path: str,
    ...
    height = {
        "144p": 144, "360p": 360, "480p": 480,
        "576p": 576, "720p": 720, "1080p": 1080,
    }.get(str(quality).lower(), 576)
...
async def _run_recording_attempt(update, context, channel, duration_seconds,
    ...
    try:
        ...
        # Same photo message becomes the processing UI.
        keyboard = _build_rec_progress_inline(session_id)
        info = RECORDING_PROGRESS_INFO.get(session_id)
        if info:
            info["status"] = "Starting Recording"
        try:
            await msg.edit_caption(
                caption=_build_process_photo_caption(info or {"filename": filename, "user_id": user_obj.id}),
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=keyboard,
            )
        except Exception:
            try:
                await msg.edit_text(_build_rec_status_text(filename, 0.0, None), parse_mode=ParseMode.MARKDOWN, reply_markup=keyboard)
            except Exception:
                pass
        ...
        preview_task = asyncio.create_task(
            _live_preview_updater(session_id, msg, RECORDING_PROGRESS_INFO[session_id])
        )
        ACTIVE_UPDATERS[session_id + ":preview"] = preview_task
        ...
        preview_task = ACTIVE_UPDATERS.pop(session_id + ":preview", None)
...
async def _ensure_rec_loading_photo() -> str:
    """Create a Telegram-compatible loading JPG from the configured loading GIF."""
    if os.path.exists(PROCESS_LOADING_PHOTO_PATH) and os.path.getsize(PROCESS_LOADING_PHOTO_PATH) > 1024:
        return PROCESS_LOADING_PHOTO_PATH
    try:
        proc = await asyncio.create_subprocess_exec(
            "ffmpeg", "-hide_banner", "-loglevel", "error", "-y",
            "-i", PROCESS_LOADING_GIF_URL,
            "-frames:v", "1", "-vf", "scale=960:-2",
            "-q:v", "3", PROCESS_LOADING_PHOTO_PATH,
            stdout=asyncio.subprocess.DEVNULL,
            stderr=asyncio.subprocess.PIPE,
        )
        _, stderr = await asyncio.wait_for(proc.communicate(), timeout=30)
        if proc.returncode == 0 and os.path.exists(PROCESS_LOADING_PHOTO_PATH):
            return PROCESS_LOADING_PHOTO_PATH
        logger.warning("Could not create loading preview photo: %s", stderr.decode(errors="replace")[-500:])
    except Exception as exc:
        logger.warning("Loading preview photo unavailable: %s", exc)
    return ""


def _rec_setup_text(pending: dict) -> str:
    audio = pending.get("selected_audio_name") or "Not selected"
    quality = pending.get("quality") or "Not selected"
    return (
        "📺 **Select a Video Track**\n\n"
        f"📄 File: `{pending.get('display_name', 'Recording')}`\n\n"
        f"🎥 Quality: **{quality}**\n"
        f"🎵 Audio: **{audio}**"
    )


def _rec_setup_keyboard(selection_id: str, pending: dict) -> InlineKeyboardMarkup:
    q = pending.get("quality")
    a = pending.get("selected_audio")
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("🎥 Quality", callback_data=f"recui:quality:{selection_id}"),
            InlineKeyboardButton("🎵 Audio", callback_data=f"recui:audio:{selection_id}"),
        ],
        [InlineKeyboardButton("🔴 Recording", callback_data=f"recui:start:{selection_id}")],
        [InlineKeyboardButton("❌ Cancel Setup", callback_data=f"recui:cancel:{selection_id}")],
    ])


def _rec_audio_keyboard(selection_id: str, tracks: list) -> InlineKeyboardMarkup:
    rows=[]
    row=[]
    for track in tracks:
        idx=int(track.get("index", 0))
        label=track.get("name") or f"Audio {idx+1}"
        row.append(InlineKeyboardButton(label, callback_data=f"recui:a:{selection_id}:{idx}"))
        if len(row)==2:
            rows.append(row); row=[]
    if row: rows.append(row)
    rows.append([InlineKeyboardButton("⬅️ Back", callback_data=f"recui:back:{selection_id}")])
    return InlineKeyboardMarkup(rows)


def _rec_quality_keyboard(selection_id: str, selected: str = "") -> InlineKeyboardMarkup:
    rows=[]
    for name in ("144p", "360p", "480p", "576p", "720p", "1080p"):
        w,h=REC_QUALITY_OPTIONS[name]
        label=f"{'✅ ' if selected==name else ''}{name} • {w}×{h} • H.264"
        rows.append([InlineKeyboardButton(label, callback_data=f"recui:q:{selection_id}:{name[:-1]}")])
    rows.append([InlineKeyboardButton("⬅️ Back", callback_data=f"recui:back:{selection_id}")])
    return InlineKeyboardMarkup(rows)


def _rec_audio_text(pending: dict) -> str:
    return "🎵 **Select Audio Track**\n\nOnly REAL audio tracks detected by FFprobe are shown."


def _rec_quality_text(pending: dict) -> str:
    return "🎥 **Select Quality**\n\nChoose the output H.264 resolution."


async def _edit_process_photo(message, photo_path: str, caption: str, reply_markup=None):
    """Replace the existing process photo in-place; never send a new message."""
    if not photo_path or not os.path.exists(photo_path):
        return False
    try:
        await message.edit_media(
            media=InputMediaPhoto(media=InputFile(photo_path), caption=caption, parse_mode=ParseMode.MARKDOWN),
            reply_markup=reply_markup,
        )
        return True
    except Exception as exc:
        logger.warning("Process photo edit failed: %s", exc)
        return False


async def _live_preview_once(info: dict) -> str:
    """Capture the current live frame from the recording source."""
    stream_url = info.get("stream_url")
    if not stream_url:
        return ""
    os.makedirs(PROCESS_PREVIEW_DIR, exist_ok=True)
    path = os.path.join(PROCESS_PREVIEW_DIR, f"{info['task_id']}.jpg")
    tmp = path + ".tmp.jpg"
    ...
    if cookie or user_agent:
        ...
        cmd += ["-headers", headers]
    if user_agent: cmd += ["-user_agent", user_agent]
    if cookie: cmd += ["-cookies", cookie]
    if info.get("cenc_key"): cmd += ["-cenc_decryption_key", info["cenc_key"]]
    cmd += ["-rw_timeout","8000000","-i",stream_url,"-map","0:v:0","-frames:v","1","-vf","scale=960:-2","-q:v","4",tmp]
    try:
        proc=await asyncio.create_subprocess_exec(*cmd, stdout=asyncio.subprocess.DEVNULL, stderr=asyncio.subprocess.PIPE)
        _,stderr=await asyncio.wait_for(proc.communicate(), timeout=18)
        if proc.returncode==0 and os.path.exists(tmp) and os.path.getsize(tmp)>1024:
            os.replace(tmp,path)
            return path
        logger.debug("Live preview capture failed: %s", stderr.decode(errors="replace")[-300:])
    except Exception as exc:
        logger.debug("Live preview exception: %s", exc)
    try: os.remove(tmp)
    except OSError: pass
    return ""


async def _live_preview_updater(task_id: str, message, info: dict):
    """Every 20 seconds replace the SAME process photo with the latest live frame."""
    try:
        while True:
            await asyncio.sleep(PROCESS_PREVIEW_INTERVAL)
            current=RECORDING_PROGRESS_INFO.get(task_id)
            if not current or not current.get("running", True):
                break
            preview=await _live_preview_once(current)
            if not preview:
                continue
            current["preview_path"]=preview
            caption=_build_process_photo_caption(current)
            await _edit_process_photo(message, preview, caption, _build_rec_progress_inline(task_id))
    except asyncio.CancelledError:
        pass


def _build_process_photo_caption(info: dict) -> str:
    filename=str(info.get("filename") or "Unknown")
    pct=float(info.get("pct") or 0.0)
    elapsed=float(info.get("elapsed") or 0.0)
    total=float(info.get("total_duration") or 0.0)
    ...
    return (
        ...
        f"📄 **File:** `{filename}`\n\n"
        f"⚡ **Speed:** `{speed_text}`\n\n"
        f"**Status:** {info.get('status','Recording')}\n"
        f"📊 **Progress:** `{pct:.2f}%`\n"
        f"⏱ **Elapsed:** `{_fmt_time(elapsed)}`\n"
        f"⏳ **Remaining:** `{_fmt_time(remaining)}`\n\n"
        f"👤 **Username:** @{info.get('username') or 'anonymous'}\n"
        f"🆔 **User ID:** `{info.get('user_id','')}`"
    )


async def rec_ui_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query=update.callback_query
    parts=query.data.split(":")
    if len(parts)<3:
        return await query.answer("Invalid recording setup.", show_alert=True, cache_time=0)
    action=parts[1]; selection_id=parts[2]
    pending=PENDING_RECORDINGS.get(selection_id)
    if not pending or pending.get("user_id")!=query.from_user.id:
        return await query.answer("Recording setup expired.", show_alert=True, cache_time=0)
    if action=="quality":
        await query.answer()
        return await query.edit_message_caption(caption=_rec_quality_text(pending), reply_markup=_rec_quality_keyboard(selection_id,pending.get("quality","")), parse_mode=ParseMode.MARKDOWN)
    if action=="audio":
        await query.answer()
        tracks=pending.get("tracks",[])
        if not tracks:
            return await query.answer("No real audio tracks detected.", show_alert=True, cache_time=0)
        return await query.edit_message_caption(caption=_rec_audio_text(pending), reply_markup=_rec_audio_keyboard(selection_id,tracks), parse_mode=ParseMode.MARKDOWN)
    if action=="back":
        await query.answer()
        return await query.edit_message_caption(caption=_rec_setup_text(pending), reply_markup=_rec_setup_keyboard(selection_id,pending), parse_mode=ParseMode.MARKDOWN)
    if action=="q" and len(parts)==4:
        key=parts[3]+"p"
        if key not in REC_QUALITY_OPTIONS:
            return await query.answer("Invalid quality.", show_alert=True, cache_time=0)
        pending["quality"]=key
        await query.answer(f"{key} selected")
        return await query.edit_message_caption(caption=_rec_setup_text(pending), reply_markup=_rec_setup_keyboard(selection_id,pending), parse_mode=ParseMode.MARKDOWN)
    if action=="a" and len(parts)==4:
        try: idx=int(parts[3])
        except ValueError: return await query.answer("Invalid audio track.", show_alert=True, cache_time=0)
        track=next((t for t in pending.get("tracks",[]) if int(t.get("index",-1))==idx),None)
        if not track:
            return await query.answer("Audio track no longer exists.", show_alert=True, cache_time=0)
        pending["selected_audio"]=idx
        pending["selected_audio_name"]=track.get("name") or f"Audio {idx+1}"
        pending["audio_mode"]="single"
        pending["audio_index"]=idx
        await query.answer(f"{pending['selected_audio_name']} selected")
        return await query.edit_message_caption(caption=_rec_setup_text(pending), reply_markup=_rec_setup_keyboard(selection_id,pending), parse_mode=ParseMode.MARKDOWN)
    if action=="cancel":
        PENDING_RECORDINGS.pop(selection_id,None)
        await query.answer("Cancelled", show_alert=True, cache_time=0)
        try: await query.message.delete()
        except Exception: pass
        return
    if action=="start":
        if pending.get("selected_audio") is None or not pending.get("tracks"):
            return await query.answer("Select a REAL audio track first.", show_alert=True, cache_time=0)
        if not pending.get("quality"):
            return await query.answer("Select a quality first.", show_alert=True, cache_time=0)
        PENDING_RECORDINGS.pop(selection_id,None)
        await query.answer("Starting recording...", cache_time=0)
        await run_recording_job(update, context, pending["channel"], pending["duration_seconds"], pending["time_range_label"], pending["aspect"], pending["quality"], pending["audio_index"], "single", pending["tracks"], query.message, past_range=pending.get("past_range"))
        return
    await query.answer("Unknown option.", show_alert=True, cache_time=0)
...
async def rec_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Direct URL mode is checked first so `/rec URL 00:00:30 ls` is accepted.
    direct_url_match = re.match(
        r"^(https?://\S+\.(?:m3u8|mpd)(?:\?[^\s]*)?)\s+(\d{1,2}:\d{2}:\d{2}|\d{1,2}:\d{2})(?:\s+ls)?$",
        full_text.strip(), re.IGNORECASE,
    )
    direct_url = None
    if direct_url_match:
        direct_url = direct_url_match.group(1)
        duration_raw = direct_url_match.group(2)
        duration_seconds = duration_value(duration_raw)
        time_range_label = duration_raw
        channel_name = Path(urlsplit(direct_url).path).stem or "Live Stream"
        source = "direct"
    ...
    # Signed provider playlist URLs can expire before the normal browse-cache TTL.
    # Direct URLs bypass provider playlist lookup.
    channel = (
        {
            "channel_name": channel_name,
            ...
            "cenc_key": "",
            "license_key": "",
        } if direct_url else find_channel(channel_name, force_refresh=True, source=source)
    )
    if not channel:
        if source == "sunnxt":
    ...
    # One photo message is created before recording setup. All setup screens
    # edit this same photo message.
    loading_photo = await _ensure_rec_loading_photo()
    if loading_photo:
        prep_message = await update.message.reply_photo(
            photo=InputFile(loading_photo),
            caption=(
                "⏳ **Loading / Setup**\n\n"
                f"📄 **File:** `{channel_name}`\n"
                f"⏱ **Duration:** `{time_range_label}`\n\n"
                "🔍 Detecting REAL audio tracks..."
            ),
            parse_mode=ParseMode.MARKDOWN,
        )
    else:
        # Photo is required by the requested UI; if the remote loading asset is
        # unavailable, keep the setup message editable rather than crashing.
    ...
    pending = PENDING_RECORDINGS[selection_id]
    # All /rec sources now use the same two-button setup screen. Provider
    # watermark menus remain available later through the existing provider flow.
    pending["display_name"] = channel.get("channel_name") or "Live Stream"
    pending["quality"] = ""
    pending["selected_audio"] = None
    pending["selected_audio_name"] = ""
    await prep_message.edit_caption(
        caption=_rec_setup_text(pending),
        reply_markup=_rec_setup_keyboard(selection_id, pending),
        parse_mode=ParseMode.MARKDOWN,
    )
...
async def rec_audio_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Direct recordings keep the SAME photo message for the processing UI.
    PENDING_RECORDINGS.pop(selection_id, None)
    status_message = query.message
    await run_recording_job(
...
async def run_recording_job(update, context, channel, duration_seconds,
    ...
    try:
        ...
        user_obj = update.effective_user
        # Persistent process state used by Progress/Cancel/Live Preview.
        RECORDING_PROGRESS_INFO[session_id] = {
            "task_id": session_id, "process": None, "start_time": time.time(),
            "duration": float(duration_seconds), "total_duration": float(duration_seconds),
            "filename": filename, "file_name": filename,
            "message_id": getattr(msg, "message_id", None),
            "chat_id": getattr(getattr(msg, "chat", None), "id", update.effective_chat.id),
            "speed": 0.0, "speed_mbps": 0.0, "platform": channel.get("source", "direct"),
            "channel": channel, "user_obj": user_obj, "user_id": user_obj.id,
            "username": user_obj.username or "anonymous", "pct": 0.0, "elapsed": 0.0,
            "status": "Starting Recording", "running": True, "phase": "recording",
            "stream_url": channel.get("stream_url", ""), "cookie": channel.get("cookie", ""),
            "user_agent": channel.get("user_agent", ""),
            "cenc_key": channel.get("cenc_key", ""), "work_dir": work_dir,
        }
        status = "failed"
        last_error = ""
        cancel_id = ""
        ...
        if status != "ok":
            ...
            try:
                await msg.edit_caption(
                    caption=fail_caption, reply_markup=None,
                    parse_mode=ParseMode.MARKDOWN,
                )
            except Exception:
                try:
                    await msg.edit_text(fail_caption, parse_mode=ParseMode.MARKDOWN)
                except Exception:
                    pass
            if work_dir and os.path.exists(work_dir):
                shutil.rmtree(work_dir, ignore_errors=True)
        ...
        # Final frame: briefly replace the SAME photo with the latest completed
        # recording preview, then automatically delete the process photo.
        thumbnail_path = await _generate_recording_thumbnail(
            out_file, thumbnail_path, duration_seconds
        )
        if thumbnail_path:
            final_info = RECORDING_PROGRESS_INFO.get(session_id) or {}
            final_info.update({"status": "Recording Finished", "pct": 100.0, "elapsed": float(duration_seconds), "total_duration": float(duration_seconds)})
            try:
                await _edit_process_photo(
                    msg, thumbnail_path,
                    _build_process_photo_caption(final_info),
                    reply_markup=None,
                )
            except Exception:
                pass
        # Recording photo message is intentionally auto-deleted before upload.
        try:
            await msg.delete()
        except Exception:
            pass
        upload_status = await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text=(
                "📦 **Uploading:**\n"
                f"`{filename}`\n\n"
            ...
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=_build_rec_progress_inline(session_id),
        )
        info = RECORDING_PROGRESS_INFO.get(session_id)
        if info:
            info["phase"] = "upload"
            info["status"] = "Uploading"
            info["upload_bytes"] = 0
            info["upload_size"] = os.path.getsize(out_file)
            info["upload_start"] = time.time()
            info["filename"] = filename
        upload_updater = asyncio.create_task(
            _auto_updater(session_id, upload_status, None, filename, duration_seconds, time.time())
        )
        ACTIVE_UPDATERS[session_id + ":upload"] = upload_updater
        try:
            upload_bot = _upload_bot(context, os.path.getsize(out_file), "Recording")
            with open(out_file, "rb") as raw:
                upload_file = _ProgressUploadFile(raw, info or {})
                telegram_input = InputFile(upload_file, filename=filename, read_file_handle=False)
                try:
                    if thumbnail_path:
                        with open(thumbnail_path, "rb") as thumbnail_file:
                            await upload_bot.send_video(
                                chat_id=update.effective_chat.id, video=telegram_input,
                                thumbnail=thumbnail_file, caption=caption,
                    ...
                    telegram_input = InputFile(upload_file, filename=filename, read_file_handle=False)
                    await upload_bot.send_document(
                        chat_id=update.effective_chat.id, document=telegram_input,
                        caption=caption, parse_mode=ParseMode.MARKDOWN,
                        read_timeout=1800, write_timeout=1800,
                    )
        finally:
            upload_updater = ACTIVE_UPDATERS.pop(session_id + ":upload", None)
            if upload_updater and not upload_updater.done():
        ...
        if preview_task and not preview_task.done():
            preview_task.cancel()
        upload_task = ACTIVE_UPDATERS.pop(session_id + ":upload", None)
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(rec_ui_callback, pattern=r"^recui:"))
