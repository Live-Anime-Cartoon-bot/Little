...
async def _shorten_url(long_url: str):
    api_key = getattr(config, "SHORTENER_API_KEY", "") or os.getenv("SHORTENER_API_KEY", "")
    api_url = getattr(config, "SHORTENER_API_URL", "https://shrinkme.io/api") or os.getenv("SHORTENER_API_URL", "https://shrinkme.io/api")
    if not api_key:
        return None
    request_url = api_url + ("&" if "?" in api_url else "?") + urllib.parse.urlencode({"api": api_key, "url": long_url})
    def request():
        req = urllib.request.Request(request_url, headers={"User-Agent": "VideoRecorderBot/1.0"})
    ...
    try:
        ...
        try:
            ...
            if isinstance(data, dict):
                for key in ("shortenedUrl", "shortened_url", "shorturl", "shortened", "url"):
                    value = data.get(key)
        ...
        LOG.error("Shortener request failed: %s", e)
        return None
...
async def start(client, message):
    ...
    await message.reply_text("🎬 **Welcome to Video Recorder Bot!**\n\n🔐 Verification is required for recording commands.\n🔑 Use /token to generate a verification link.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔑 Token Generator", callback_data="token_generate")],[InlineKeyboardButton("📖 Help", callback_data="help")],[InlineKeyboardButton("💠 Plans", callback_data="plan")],[InlineKeyboardButton("📢 Channel", url="https://t.me/ToonixIndia")]]))
...
async def token_command(client, message):
    ...
    await message.reply_text("🔐 **Verification Required**\n\nClick the button below to generate your verification link.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔑 Token Generator", callback_data="token_generate")]]))
...
async def token_generate_callback(client, query):
    ...
    await query.answer("Generating verification link...")
    token = _new_verification_token(user_id)
    me = await client.get_me()
    deep_link = f"https://t.me/{me.username}?start=verify_{token}"
    short_url = await _shorten_url(deep_link)
    if not short_url:
        return await query.message.reply_text("❌ **Shortener is not configured or unavailable.**\n\nAdd `SHORTENER_API_KEY` and optionally `SHORTENER_API_URL` to config.py.")
    await query.message.reply_text("🔑 **Verification Link Generated**\n\nComplete the shortener verification, then the bot will verify your deep-link.\n\n⏳ Access: **6 hours**.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔐 Verify Now", url=short_url)]]))
